...
public class WindSlash : NetworkBehaviour, Projectile
{
    ...
    private void Move(float deltaTime)
    {
        ...
        if (Physics.SphereCast(transform.position, _colliderRadius, transform.TransformDirection(Vector3.forward), out hit, traceDistance) && !isExploding)
        {
            if (hit.transform.tag == "Player" && hit.transform.parent.GetComponent<NetworkObject>().Owner.ClientId != owner)
            {
                ...
                explode(true, transform.position);
                isExploding = true;
            }
        }
        ...
        {
            explode(false, transform.position);
            isExploding = true;
        }
    }

    [Server(Logging = LoggingType.Off)]
    private void explode(bool hit, Vector3 pos)
    {
        if (base.IsServer)
        {
            ObserversSpawnExplodePrefab(hit, pos);
    ...
    /// </summary>
    [ObserversRpc]
    private void ObserversSpawnExplodePrefab(bool hit, Vector3 pos)
    {
        if (hit)
    ...
    {
        GameObject spawned = Instantiate(hitImpact, pos, transform.rotation);
